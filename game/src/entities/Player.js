...
export class Player {
  ...
  /**
   * Переключение на анимацию idle (стояние на месте)
   * Используется при паузе туториала
   */
  idle() {
    if (!this.sprite) return
    
    // Переключаем на idle анимацию (анимация продолжает проигрываться, но игрок стоит на месте)
    ...
    // Помечаем что игра на паузе (чтобы не обновлять движение)
    this.gameStarted = false
    
    console.log('⏸️ Игрок переключен на idle (пауза)')
  }
